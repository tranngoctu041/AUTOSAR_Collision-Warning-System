...
#define SCHM_LIN_SW_MAJOR_VERSION_C             1
#define SCHM_LIN_SW_MINOR_VERSION_C             0
#define SCHM_LIN_SW_PATCH_VERSION_C             1
...
void SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_26(void)
{
    ...
    {
#if (defined MCAL_ENABLE_USER_MODE_SUPPORT)
        msr_LIN_EXCLUSIVE_AREA_26[u32CoreId] = OsIf_Trusted_Call_Return(Lin_schm_read_msr);
#else
        msr_LIN_EXCLUSIVE_AREA_26[u32CoreId] = Lin_schm_read_msr();  /*read MSR (to store interrupts state)*/
#endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
        if (ISR_ON(msr_LIN_EXCLUSIVE_AREA_26[u32CoreId])) /*if MSR[EE] = 0, skip calling Suspend/Resume AllInterrupts*/
        {
            OsIf_SuspendAllInterrupts();
#ifdef _ARM_DS5_C_S32K1XX_
            ASM_KEYWORD(" nop ");/* Compiler fix - forces the CSPID instruction to be generated with -02, -Ospace are selected*/
...
void SchM_Enter_Lin_LIN_EXCLUSIVE_AREA_27(void)
{
    ...
    {
#if (defined MCAL_ENABLE_USER_MODE_SUPPORT)
        msr_LIN_EXCLUSIVE_AREA_27[u32CoreId] = OsIf_Trusted_Call_Return(Lin_schm_read_msr);
#else
        msr_LIN_EXCLUSIVE_AREA_27[u32CoreId] = Lin_schm_read_msr();  /*read MSR (to store interrupts state)*/
#endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
        if (ISR_ON(msr_LIN_EXCLUSIVE_AREA_27[u32CoreId])) /*if MSR[EE] = 0, skip calling Suspend/Resume AllInterrupts*/
        {
            OsIf_SuspendAllInterrupts();
#ifdef _ARM_DS5_C_S32K1XX_
            ASM_KEYWORD(" nop ");/* Compiler fix - forces the CSPID instruction to be generated with -02, -Ospace are selected*/
