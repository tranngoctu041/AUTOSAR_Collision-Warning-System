...
#define SCHM_CRC_SW_MAJOR_VERSION_C             1
#define SCHM_CRC_SW_MINOR_VERSION_C             0
#define SCHM_CRC_SW_PATCH_VERSION_C             1
...
void SchM_Enter_Crc_CRC_EXCLUSIVE_AREA_00(void)
{
    ...
    if(0UL == reentry_guard_CRC_EXCLUSIVE_AREA_00[u32CoreId])
    {
        ...
        msr_CRC_EXCLUSIVE_AREA_00[u32CoreId] = OsIf_Trusted_Call_Return(Crc_schm_read_msr);
#else
        msr_CRC_EXCLUSIVE_AREA_00[u32CoreId] = Crc_schm_read_msr();  /*read MSR (to store interrupts state)*/
#endif /* MCAL_ENABLE_USER_MODE_SUPPORT */
        if (ISR_ON(msr_CRC_EXCLUSIVE_AREA_00[u32CoreId])) /*if MSR[EE] = 0, skip calling Suspend/Resume AllInterrupts*/
        {
