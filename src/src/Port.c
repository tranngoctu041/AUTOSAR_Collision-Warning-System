...
#if (PORT_VENDOR_ID_C != PORT_VENDOR_ID_H)
...
#if ((PORT_AR_RELEASE_MAJOR_VERSION_C    != PORT_AR_RELEASE_MAJOR_VERSION_H) || \
     (PORT_AR_RELEASE_MINOR_VERSION_C    != PORT_AR_RELEASE_MINOR_VERSION_H) || \
     (PORT_AR_RELEASE_REVISION_VERSION_C != PORT_AR_RELEASE_REVISION_VERSION_H) \
    )
...
#if ((PORT_SW_MAJOR_VERSION_C != PORT_SW_MAJOR_VERSION_H) || \
     (PORT_SW_MINOR_VERSION_C != PORT_SW_MINOR_VERSION_H) || \
     (PORT_SW_PATCH_VERSION_C != PORT_SW_PATCH_VERSION_H)    \
    )
...
static const Port_ConfigType * Port_pConfig;
...
)
{
    ...
    else if ((uint32)1 != (((Port_pConfig->pau32Port_PinToPartitionMap[Pin])&((uint32)1 << CoreId)) >> CoreId))
    {
    ...
    {
#if (STD_ON == PORT_DEV_ERROR_DETECT)
        ErrStatus = Port_Ipw_SetPinDirection(Pin, Direction, Port_pConfig);
#else
        (void)Port_Ipw_SetPinDirection(Pin, Direction, Port_pConfig);
#endif
...
)
{
    ...
    else if(FALSE ==  Port_pConfig->pUsedPadConfig[Pin].bMC)
    {
...
void Port_RefreshPortDirection( void )
{
    ...
    else if ((uint32)1 != Port_pConfig->pau8Port_PartitionList[CoreId])
    {
...
)
{
...
#endif /* (STD_ON == PORT_DEV_ERROR_DETECT) */
    {
        ...
        (versioninfo)->sw_major_version = (uint8)PORT_SW_MAJOR_VERSION_H;
        (versioninfo)->sw_minor_version = (uint8)PORT_SW_MINOR_VERSION_H;
        (versioninfo)->sw_patch_version = (uint8)PORT_SW_PATCH_VERSION_H;
    }
}
...
#endif
    {
        Port_Ipw_SetAsUnusedPin(Pin, Port_pConfig);
    }
}
...
#endif
    {
        Port_Ipw_SetAsUsedPin(Pin, Port_pConfig);
    }
}
...
#endif
    {
        Port_Ipw_ResetPinMode(Pin, Port_pConfig);
    }
}
